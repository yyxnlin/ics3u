...
public class ArraysAssignment4 {
	...
	// Description: generates an array of doubles of a certain size within an inclusive range rounded to 1 decimal place, excluding 0
	// Parameters: double max is max value, double min is min value, int size is number of elements
	// Return: array of doubles rounded to 1 decimal place
	public static double[] generateArray(double max, double min, int size) {
		...
		for (int i = 0; i < size; i ++) {
			...
			// if generates 0, retry
			while (random == 0) {
				random = (int)(Math.random()*(max*10 - min*10 + 1)) + (int) (min*10); 
			}
	...
	// Description: gives String representation of array in neat columns of specified size
	// Parameters: double arr[] is the given array, int numPerRow is the number of elements per row 
	// Return: String containing values of array printed in neat columns
	public static String displayArray(double arr[], int numPerRow) {
		String output = "";
		int numInRow = 0; // which index per row the number is at
		int rowNum = 0; // which row the number is at

		for(int i = 0; i < arr.length; i++) { // i keeps track of current index in whole array
			if (numInRow < numPerRow) { // hasn't reached end of row
				output += String.format("%8.1f", arr[rowNum*numPerRow + numInRow]); // add current value in array to the string
				numInRow++;

			}
			else { // reached end of row -> start new row
				rowNum++;
				numInRow = 0;
				i--; // doesn't add current number in loop to the array, so subtract 1 to loop correct # of times
				output += "\n";
			}
	...
	// Description: finds the range (difference between smallest and largest value) of array
	// Parameters: double[] arr is the given array
	// Return: double containing the range of the array
	public static double rangeOfArray(double[] arr) {
		...
		// range is difference between max and min rounded to one decimal place
		return (Math.round(10*(max-min))/10.0);
	}
	
	// Description: finds all the numbers in the array that is greater than half the range
	// Parameters: double[] arr is the given array
	// Return: array of doubles containing the values from the array that is greater than half the range
	...
	// Description: sorts double array from smallest -> largest by swapping smallest element in each iteration with the beginning of unsorted array
	// Parameters: double[] arr is the given array
	// Return: no return; modifies original array to become sorted array of doubles 
	...
	// Description: merges two already sorted double arrays into a combined sorted double array
	// Parameters: double[] arr1, double[] arr2 are the two original sorted arrays
	// Return: sorted double array containing elements of arr1 and arr2 combined
	...
	// Description: removes all occurrences of one array in another
	// Parameters: double[] arr1 is original array, double[] arr2 is sublist to be removed from original
	// Return: new double array with all occurrences of arr2 removed from arr1
	public static double[] removeSublist(double[] arr1, double[] arr2) {
		...
		// make a copy of arr1 so that it doesn't modify original array
		...
		for (int i = 0; i <= origArr.length - arr2.length; i++) {
			...
			// if it is found (never broke from above section), set all values of arr2 inside origArr to 0
			if (found) {
				for (int j = i; j < i + arr2.length; j++) {
					origArr[j] = 0;
				}
				i += arr2.length - 1; // continue searching for arr2 after the sublist in arr1
				count++;
			}
		}
		...
		// copy all elements from origArr to newArr if value is not 0 (which would be excluded)
	...
	public ArraysAssignment4() {
		...
		btnSublist.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// get new array with sublist removed + display it in mainText
				double[] modifiedArray = removeSublist(arr1, arr2);
				mainText.setText(displayArray(modifiedArray, numPerRow));
	...
	void initJComponents() {
		...
		inputPanel2.add(new JLabel("Removed arr2 size: "));
